...
sigma.default = function(Y, N0, T0) {
  sd(apply(Y[1:N0, 1:T0], 1, diff))
}
epsilon = 1e-6

# %%
synthdid.reference = function(Y, N0, T0,
    zeta.omega = ((nrow(Y) - N0) * (ncol(Y) - T0))^(1 / 4) * sigma.default(Y, N0, T0)
  ) {
  N = nrow(Y); T = ncol(Y); N1 = N - N0; T1 = T - T0;
  lambda = simplex.least.squares(Y[1:N0, 1:T0],
                                rowMeans(Y[1:N0, (T0 + 1):T, drop = FALSE]),
                                zeta = epsilon * sigma.default(Y, N0, T0),
                                intercept = TRUE)
  omega  = simplex.least.squares(t(Y[1:N0, 1:T0]),
